...
function installFire(){
	echo "whats up" | whiptail --textbox 0 0
	#installer="install_fire_$fireVersion.sh"
	#wget "https://github.com/shaglama/fire/blob/development/$installer"
	#chmod +x "$installer"
	#/bin/bash "$installer" --options="apiKey=$apiKey;walletSecret=$walletSecret;forceScan=$forceScan;forceValidate=$forceValidate;useSnapshot=$useSnapshot;snapshotUrl=$snapshotUrl"
	#rm $installer
}
########## PROGRAM ##################################################
fireVersion="0.0.3.0"
